...
type Props = {
    ...
    // Callback để truyền tên topic ra ngoài cho Page hiển thị
    onTopicNameChange?: (name: string) => void;
};

const TopicWeightHistory: React.FC<Props> = ({ topicWeightId, items, onTopicNameChange }) => {
    const router = useRouter(); // Hook để back trang
    const params = useParams() as any;
    const courseCodeId = params?.id as string | undefined;

    // 1. Hook lấy danh sách Topic (Dropdown)
    const { data: topicsResp, loading: topicsLoading, fetchTopicWeights } = useGetTopicWeights();
    const topicWeights = topicsResp?.data ?? [];
    ...
    // --- EFFECT: Fetch Topics ---
    useEffect(() => {
        if (courseCodeId) fetchTopicWeights({ courseCodeId });
    ...
    // Logic xác định tên cột (Header): Nếu có item nào có specificCourseName thì ưu tiên hiển thị cột đó
                    ...
                    <Table>
                        <TableHeader className="bg-gray-50">
                            <TableRow>
                                ...
                                {/* Hiển thị Dynamic Header dựa trên dữ liệu */}
                            ...
                            {isLoading ? (
                                ...
                                displayItems.map((it) => (
                                    <TableRow key={it.id} className="hover:bg-slate-50/50 transition-colors">
                                        ...
                                        {/* Context (Course Code OR Specific Course) */}
                                        <TableCell className="align-top py-4">
                                            <div className="flex flex-col text-sm">
                                                <span className="font-medium text-slate-800 break-words">
                                                    {/* Ưu tiên hiển thị specific nếu có, nếu ko thì course code */}
