  ...
  return (
    ...
    >
      <header className="h-20 border-b border-gray-200 flex items-center px-4">
        <div className={clsx("flex items-center gap-2 w-full", collapsed ? "justify-center" : "justify-between")}>
          {!collapsed && (
            <div className="flex items-center gap-2">
              <GraduationCap className="w-5 h-5 text-blue-600" />
              <div>
                <p className="text-lg font-semibold text-gray-900">Staff Panel</p>
          ...
          >
            <ChevronRight className={clsx("text-white transition-transform duration-500 w-5 h-5", collapsed ? "rotate-0" : "rotate-180")} />
          </Button>
        </div>
      </header>

      <nav className="flex-1 overflow-y-auto p-4">
        {!collapsed && <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Main</p>}
        ...
        <ul className="space-y-2">
          {mainNav.map((item) => {
            ...
            const renderChildren = (items: any[], depth = 1) => {
              ...
              // [THUẬT TOÁN] Tìm trong đám anh em, thằng nào có href khớp với URL hiện tại DÀI NHẤT
              let bestMatchHref = "";
              let maxLen = 0;
              ...
              return (
                <ul className={clsx("mt-2 space-y-1 border-l border-blue-100", depth === 1 ? "pl-2 ml-4 border-l border-blue-200" : "pl-2 ml-4")}>
                  {items.map((c) => {
                    ...
                    // [ÁP DỤNG FIX] Nếu tìm được "Best Match" trong nhóm này
                    if (bestMatchHref) {
                      // Chỉ active thằng có href trùng với bestMatchHref
                      childActive = (c.href === bestMatchHref);
                    }
                    ...
                    return (
                      <li key={c.href}>
                        <Link href={c.href} className={clsx(linkBase, childActive ? linkActiveClass : linkInactiveClass)}>
                          {isThird ? (
                            <span className={clsx("w-2 h-2 rounded-full mr-2 mt-1", childActive ? "bg-blue-600" : "bg-gray-300")} />
                          ) : (
                            <span className={clsx("w-1 h-6 rounded mr-2 self-start mt-1", childActive ? "bg-blue-600" : "bg-gray-300")} />
                          )}
                          ...
                          {childHasChildren && (
                            ...
                            >
                              <ChevronRight className={clsx("w-3 h-3 transition-transform duration-300", childExpanded ? "rotate-90" : "rotate-0", childActive ? "text-blue-700" : "text-gray-400")} />
                            </button>
                          )}
                        </Link>
                        {c.children && c.children.length > 0 && (
                          <div className={clsx("overflow-hidden transition-all duration-500 ease-in-out", childExpanded ? "max-h-[1000px] opacity-100 mt-1" : "max-h-0 opacity-0")}>
            ...
            return (
              <li key={href}>
                ...
                >
                  <Icon className={clsx("transition-transform group-hover:scale-110 shrink-0 w-5 h-5", active ? "text-white" : "text-gray-500 group-hover:text-blue-600")} />
                  {!collapsed && (
                    <div className="flex-1 flex flex-col min-w-0">
                      <span className={clsx("font-medium text-sm truncate", active ? "text-white" : "group-hover:text-blue-600")}>{label}</span>
                      {description && <p className={clsx("text-xs mt-0.5 truncate", active ? "text-white/80" : "text-gray-500 group-hover:text-blue-500")}>{description}</p>}
                    </div>
                  )}
                  {hasChildren && !collapsed && (
                    ...
                    >
                      <ChevronRight className={clsx("w-4 h-4 transition-transform duration-300", isExpanded ? "rotate-90" : "rotate-0", active ? "text-white" : "text-gray-400")} />
                    </button>
                  )}
                  {active && !collapsed && <span className="absolute right-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-white rounded-l-full" />}
                </Link>
                {!collapsed && children && children.length > 0 && (
                  <div className={clsx("overflow-hidden transition-all duration-500 ease-in-out", isExpanded ? "max-h-[2000px] opacity-100" : "max-h-0 opacity-0")}>
