  ...
  if (!course || !stats) {
    return (
      <div className="p-6 text-center text-slate-500 space-y-4">
        ...
        <div>
          <Button onClick={() => router.push("/staff/manager/courses")}>← Back</Button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-5 space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => router.push("/staff/manager/courses")} className="rounded-xl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <MetricTile
          href={`/staff/manager/courses/${courseId}/total-enrollments`}
          icon={<Users className="size-4" />}
          label="Total Enrollments"
          value={stats.totalEnrollments}
        />
        <MetricTile
          href={`/staff/manager/courses/${courseId}/total-assignments`}
          icon={<ClipboardList className="size-4" />}
          label="Total Assignments"
          value={stats.totalAssignments}
        />
        <MetricTile
          href={`/staff/manager/courses/${courseId}/total-groups`}
          icon={<LayoutGrid className="size-4" />}
          label="Total Groups"
          value={stats.totalGroups}
        />
        <MetricTile
          href={`/staff/manager/courses/${courseId}/chat-messages`}
          icon={<MessageSquare className="size-4" />}
          label="Chat Messages"
          value={stats.totalChatMessages}
        />
        <MetricTile
          href={`/staff/manager/courses/${courseId}/notifications`}
          icon={<Bell className="size-4" />}
          label="Notifications"
          value={stats.totalNotifications}
        />
        <MetricTile
          href={`/staff/manager/courses/${courseId}/recent-enrollments`}
