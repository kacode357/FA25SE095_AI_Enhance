...
export default function SupportRequestMessageList({
    ...
    return (
        <div
            ...
            className="flex-1 overflow-y-auto space-y-4 bg-[var(--background)]"
            style={{
        ...
        >
                    ...
                    if (it.kind === "sep") {
                        return (
                            <div key={it.id} className="w-full flex items-center py-3">
                                ...
                                <div className="mx-3 px-3 py-1 bg-[var(--card)] rounded-full text-xs text-[var(--text-muted)] shadow-sm">
                                    {it.label}
                                </div>
                                <div className="flex-1 border-t border-[var(--border)]" />
                            </div>
                            ...
                            >
                                {isMe && (
                                    <div className="relative">
                                        ...
                                        >
                                            <EllipsisVertical className="w-4 h-4 text-[var(--text-muted)]" />
                                        </button>

                                        <div
                                            data-menu-owner={m.id}
                                            className={`absolute -left-12 -top-2 bg-[var(--card)] border-[var(--border)] border shadow-md rounded-md p-1 z-50 transition-transform ${openMenuId === m.id ? "opacity-100 pointer-events-auto translate-y-0" : "opacity-0 pointer-events-none -translate-y-1"}`}
                                        >
                                            <button
                                ...
                                {isSystem && !isResolved && typeof m.message === "string" && m.message.includes("Has your need been resolved") ? (
                                    ...
                                    <div
                                        className={
                                            isSystem
                                                ? "rounded-full px-4 py-2 text-sm leading-relaxed inline-block whitespace-pre-wrap text-[var(--text-muted)] bg-[rgba(127,113,244,0.08)] shadow-sm max-w-[70%] text-center"
                                                : `rounded-2xl px-4 py-3 text-sm leading-relaxed inline-block whitespace-pre-wrap break-words ${isMe
                                                    ? "bg-[var(--brand)] text-white shadow-md max-w-[70vw] mr-6"
                                                    : "border border-[var(--border)] bg-white text-slate-900 max-w-[70vw] min-w-0 break-words shadow-sm ml-6"
                                                }`
                                        }
                                    >
                                ...
                                <div>
                                    {hoverPos.id === m.id && (
                                        ...
                                        >
                                            <div className="bg-[var(--card)] border border-[var(--border)] shadow-sm rounded-md px-3 py-1 text-xs text-[var(--text-muted)]">
                                                {formatted}
                                            </div>
                                        </div>
                                    )}
            ...
            {peerTyping && (
                <div className="px-6 pt-1 text-[11px] text-[var(--text-muted)]">{peerName || "User"} is typing...</div>
            )}
        </div>
    );
}
