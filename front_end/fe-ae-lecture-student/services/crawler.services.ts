...
/**
 * NOTE:
 * - Dùng crawlAxiosInstance vì base API cùng domain. Nếu BE tách service khác,
 *   đổi sang crawlerAxiosInstance ở đây là xong.
 */
export const CrawlerService = {
  /** POST /api/Crawler/start */
  start: async (payload: StartCrawlerPayload): Promise<StartCrawlerResponse> => {
    const res = await crawlAxiosInstance.post<StartCrawlerResponse>("/Crawler/start", payload);
    return res.data;
  },

  /** GET /api/Crawler/{jobId}/status?userId=... */
  status: async (jobId: string, userId?: string): Promise<GetCrawlerStatusResponse> => {
    const res = await crawlAxiosInstance.get<GetCrawlerStatusResponse>(`/Crawler/${jobId}/status`, {
  ...
  cancel: async (jobId: string, userId?: string): Promise<CancelCrawlerResponse> => {
    const res = await crawlAxiosInstance.post<CancelCrawlerResponse>(`/Crawler/${jobId}/cancel`, null, {
      params: userId ? { userId } : undefined,
    });
    return res.data;
  },
  ...
  listUserJobs: async (args: {
    ...
    const res = await crawlAxiosInstance.get<ListCrawlerJobsResponse>(`/Crawler/user/${userId}/jobs`, {
