...
export default function MessageThread({ conversation, userId }: Props) {
    const { user } = useAuth();
    ...
    const handleReceiveMessage = useCallback((msg: ChatMessageItemResponse) => {
        ...
        if (!conv) return;
        // Only accept messages belonging to the active pair (current user <-> selected other user)
        const otherId = conv.otherUserId || userId || null;
        if (!otherId || !currentUser?.id) return;
        const belongsToPair =
            (msg.senderId === currentUser.id && String(msg.receiverId) === String(otherId)) ||
            (String(msg.senderId) === String(otherId) && msg.receiverId === currentUser.id);
    ...
    const handleReceiveMessagesBatch = useCallback((batch: ChatMessageItemResponse[]) => {
        ...
        const currentUser = userRef.current;
        const otherId = conv?.otherUserId || userId || null;
        if (!conv || !batch?.length || !otherId || !currentUser?.id) return;
        setMessages((prev) => {
            const byId = new Map(prev.map((m) => [m.id, m]));
            for (const it of batch) {
                const belongsToPair =
                    (it.senderId === currentUser.id && String(it.receiverId) === String(otherId)) ||
                    (String(it.senderId) === String(otherId) && it.receiverId === currentUser.id);
                if (belongsToPair && !byId.has(it.id)) byId.set(it.id, it);
            }
    ...
    useEffect(() => {
        let mounted = true;
        (async () => {
            // Only fetch when both conversationId and other user match
            if (!conversation?.id) {
                setMessages([]);
                setPageNumber(1);
                return;
            }
            if (conversation?.otherUserId && userId && String(conversation.otherUserId) !== String(userId)) {
                // mismatch: clear messages and avoid loading wrong conversation
                setMessages([]);
                setPageNumber(1);
                return;
            }
    ...
    const { connect, disconnect, sendMessage, markMessagesAsRead } = useChatHub({
    ...
    useEffect(() => {
        ...
        (async () => {
            const otherId = conversation?.otherUserId || userId;
            if (!otherId) {
    ...
    }, [conversation?.otherUserId, userId, connect, disconnect]);

    // Mark messages as read when opening a conversation and messages exist (only for matching otherId)
    useEffect(() => {
        const convId = conversation?.id;
        if (!convId) return;
        if (messages.length === 0) return;
        if (conversation?.otherUserId && userId && String(conversation.otherUserId) !== String(userId)) return;
        markMessagesAsRead(convId).catch(() => { /* ignore */ });
    ...
    const onSend = async () => {
        ...
        const local = {
            ...
            receiverId: conversation?.otherUserId || userId || "",
        ...
        if (local.receiverId) {
            pendingRef.current.set(tempId, { createdAt: Date.now(), message, receiverId: local.receiverId });
        }
        ...
        <Card className="h-full flex border-slate-300 -py-6 flex-col">
            ...
            <div className="flex items-center justify-between border-b border-slate-300 px-3 py-2">
                <div className="flex items-start gap-3 py-2">
                    ...
                    <div>
                        <div className="text-sm font-semibold text-nav">Chat Conversation</div>
                        <div className="text-xs mt-1 text-[var(--text-muted)]">Chat with <span className="font-medium">{conversation?.otherUserName || "Unknown"}</span></div>
                        {/* <div className="text-xs text-[var(--text-muted)] mt-0.5">
                            userId: <span className="font-mono">{userId || conversation?.otherUserId || "-"}</span> · conversationId: <span className="font-mono">{conversation?.id || "(new)"}</span>
                        </div> */}
                    </div>
                </div>
            </div>
                ...
                <ScrollArea className="h-full">
                    <div className="p-3 space-y-3">
                        {loading && messages.length === 0 ? (
                            ...
                            <div className="text-sm italic text-slate-500 text-muted-foreground">No messages yet.</div>
                        ) : (
                            <>
                                {hasMore && (
