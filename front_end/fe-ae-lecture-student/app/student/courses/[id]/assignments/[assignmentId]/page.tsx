...
export default function AssignmentDetailPage() {
  ...
  const chips = [
    ...
    ...(a.isGroupAssignment
      ? [{ icon: <Users className="w-3 h-3" />, label: "Group", className: "bg-indigo-50 text-indigo-700 border border-indigo-200" }]
      : []),
  ];
            ...
            <div className="text-sm text-foreground/80 space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <b>{finalDue ? dueLabel : "Due"}:</b>&nbsp;{finalDue ? dt(finalDue.toISOString()) : "—"}
                </div>
                <div className="flex items-center gap-2">
              ...
              {(a.format || a.gradingCriteria) && (
                <div className="border-t pt-3 mt-2 grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <div className="text-xs uppercase tracking-wide text-[var(--text-muted)] mb-1">Format Submit</div>
                    <div>{a.format || "—"}</div>
                  </div>
                  <div>
                    <div className="text-xs uppercase tracking-wide text-[var(--text-muted)] mb-1">Grading Criteria</div>
          ...
          <div className="card rounded-2xl p-4">
            ...
            <div className="text-sm text-foreground/80 space-y-2">
              ...
              <div className="pt-1 text-xs text-[var(--text-muted)]">
                Created: {dt(a.createdAt)}{a.updatedAt && <> • Updated: {dt(a.updatedAt)}</>}
              </div>
            </div>
          </div>
