      ...
      <div className="card p-5 relative z-10">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2 pt-1">
            {promptUsed && (
              <div className="flex items-start gap-2 rounded-lg bg-slate-50 p-2 text-[11px] text-[var(--text-muted)] border border-[var(--border)]">
                <History className="h-3.5 w-3.5 mt-0.5 shrink-0" />
                <p>
                  <span className="font-medium text-slate-700">Last prompt:</span>{" "}
                  {promptUsed}
                </p>
              </div>
            )}

            {/* Hiển thị Active Job ID ở Card Chính (giữ nguyên) */}
            {activeJobId && !isCrawling && (
              <div className="flex items-center gap-2 text-[11px] text-[var(--brand)] font-medium">
                <CheckCircle2 className="h-3.5 w-3.5" />
                <p>
                  Last job: <code className="bg-[var(--brand)]/10 px-1 py-0.5 rounded">{activeJobId.substring(0, 8)}...</code> completed.
                </p>
              </div>
            )}
          </div>
