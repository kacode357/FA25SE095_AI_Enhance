...
import { BookOpen, CalendarDays, Loader2, LogOut, MoreVertical, PlayCircle, Users } from "lucide-react";
...
const CourseCard = ({
  ...
  const imgUrl = course.img || DEFAULT_IMG;
  ...
  useEffect(() => {
    ...
    loader.onerror = () => {
      if (mounted) setBgUrl(DEFAULT_IMG);
    };
    return () => {
      mounted = false;
    };
  ...
  return (
    <motion.article
      ...
      className="card overflow-hidden p-0 flex flex-col sm:flex-row group hover:border-[var(--brand-600)] transition-colors rounded-lg"
    >
      {/* Ảnh thu nhỏ (Compact) - make it fill and clip to card rounded corner */}
  <div className="relative w-full sm:w-[45%] md:w-[45%] lg:w-[30%] shrink-0 overflow-hidden rounded-l-lg">
        <div className="aspect-video sm:h-full sm:aspect-auto relative">
          <div
            ref={bgRef}
            role="img"
