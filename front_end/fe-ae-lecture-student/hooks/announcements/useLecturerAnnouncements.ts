// hooks/announcements/useLecturerAnnouncements.ts
"use client";
...
export function useLecturerAnnouncements() {
  ...
  const fetchLecturerAnnouncements = useCallback(
    async (
      params?: GetAnnouncementsQuery
    ): Promise<GetAnnouncementsResponse> => {
      setLoading(true);
      try {
        const res = await AnnouncementService.getLecturerAnnouncements(params);
        const page = res.data;
