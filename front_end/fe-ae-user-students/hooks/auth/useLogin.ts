...
import { mapRole, ALLOWED_LOGIN_ROLES, UserRole } from "@/config/user-role";
...
export function useLogin() {
  ...
  const login = async (
    ...
    try {
      ...
      console.log("User role enum:", roleEnum);

      const isAllowedRole =
        roleEnum !== null && ALLOWED_LOGIN_ROLES.includes(roleEnum);

      if (roleEnum === null || !isAllowedRole) {
        // Không đúng role được phép → xoá token + báo lỗi
        Cookies.remove("accessToken");
        Cookies.remove("refreshToken");
        sessionStorage.removeItem("accessToken");
        toast.error("Chỉ học viên (Student) mới được phép đăng nhập!");
        return null;
      }
