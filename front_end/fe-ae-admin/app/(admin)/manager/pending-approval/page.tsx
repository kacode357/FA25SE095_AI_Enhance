...
import { PendingUser } from "@/types/admin/admin.response";
...
export default function PendingApprovalPage() {
  ...
  const handleReject = async () => {
    ...
    const res = await suspendUser(rejectUserId, {
      ...
      suspendUntil: suspendUntil || new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(),
    });
    if (res?.success) {
      setRejectOpen(false);
      setReason("");
  ...
  const filtered = useMemo(() => {
    ...
    return data.users.filter((u: PendingUser) => {
            ...
            <div className="h-full overflow-auto">
              <Table className="table-auto w-full">
                <TableHeader className="sticky top-0 z-10 bg-slate-50">
                  {/* ... giữ nguyên table head + filter row ... */}
                </TableHeader>
                <TableBody>
  ...
  {/* Không được render trực tiếp DialogContent */}
