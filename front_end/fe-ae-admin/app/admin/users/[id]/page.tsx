...
export default function AdminUserDetailPage() {
  ...
  const quotaPercentage = user.crawlQuotaLimit > 0 
    ? (user.crawlQuotaUsed / user.crawlQuotaLimit) * 100 
    : 0;

  // 🟢 Check xem có phải Lecturer không để ẩn Quota
          ...
          <div className="rounded-xl border border-[var(--border)] bg-white p-6 shadow-sm flex flex-col items-center text-center">
            ...
            <div className="w-full space-y-3 text-left">
              <div className="flex justify-between text-sm">
                ...
                <Badge variant={user.isEmailConfirmed ? "default" : "secondary"} className={user.isEmailConfirmed ? "bg-emerald-600 text-white hover:bg-emerald-700" : ""}>
              ...
              {user.emailConfirmedAt && (
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-slate-900">{formatDateTimeVN(user.emailConfirmedAt)}</span>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="rounded-xl border border-[var(--border)] bg-white p-6 shadow-sm">
            <h3 className="flex items-center gap-2 font-semibold text-slate-900 mb-4">
              ...
              Subscription {isLecturer ? "" : "& Usage"} {/* Đổi title nếu là Lecturer */}
            </h3>
            
            {/* Nếu là Lecturer thì chỉ hiện 1 cột, còn lại thì hiện 2 cột (Sub + Quota) */}
            <div className={`grid gap-6 ${isLecturer ? 'grid-cols-1' : 'sm:grid-cols-2'}`}>
              <div className="space-y-4">
                <div>
                  <span className="text-xs font-medium uppercase text-slate-500">Current Tier</span>
                  <div className="mt-1">
                    <Badge variant="outline" className="text-sm px-3 py-1 border-brand/30 bg-brand/5 text-brand">
                ...
                <div className="grid grid-cols-2 gap-4">
                   <div>
                     ...
                     <span className="text-sm font-medium">{user.subscriptionStartDate ? formatDateTimeVN(user.subscriptionStartDate) : "N/A"}</span>
                   </div>
                   <div>
                     <span className="text-xs text-slate-500 block">End Date</span>
                     <span className="text-sm font-medium">{user.subscriptionEndDate ? formatDateTimeVN(user.subscriptionEndDate) : "N/A"}</span>
                   </div>
                </div>
              </div>

              {/* 🟢 QUOTA SECTION: Chỉ hiện nếu KHÔNG phải Lecturer */}
              {!isLecturer && (
                <div className="rounded-lg bg-slate-50 p-4 border border-slate-100">
                   <div className="flex justify-between items-end mb-2">
                      <span className="text-sm font-medium text-slate-700">Crawl Quota Usage</span>
                   ...
                   <p className="text-xs text-slate-400 mt-2">
                     Resets on: {user.quotaResetDate ? formatDateTimeVN(user.quotaResetDate) : "N/A"}
                   </p>
                </div>
              )}
            </div>
          ...
          <div className="rounded-xl border border-[var(--border)] bg-white p-6 shadow-sm">
            ...
            <div className="grid gap-y-4 sm:grid-cols-2 sm:gap-x-8">
               <div>
                  <span className="text-xs text-slate-500 block">Institution Name</span>
                  <p className="text-sm font-medium text-slate-900 mt-1">{user.institutionName || "Not Provided"}</p>
               </div>
               <div>
                  <span className="text-xs text-slate-500 block">Department</span>
                  <p className="text-sm font-medium text-slate-900 mt-1">{user.department || "Not Provided"}</p>
               </div>
               <div>
                  <span className="text-xs text-slate-500 block">Student/Staff ID</span>
                  <p className="text-sm font-medium text-slate-900 mt-1">{user.studentId || "N/A"}</p>
               </div>
               <div className="sm:col-span-2">
                  <span className="text-xs text-slate-500 block">Address</span>
                  <div className="flex gap-2 mt-1">
                     <MapPin className="h-4 w-4 text-slate-400 shrink-0 mt-0.5" />
                     <p className="text-sm text-slate-900">{user.institutionAddress || "Not Provided"}</p>
                  </div>
               </div>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-[var(--border)] bg-white p-6 shadow-sm">
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              ...
              <div>
                <span className="text-xs text-slate-500 block mb-1">Updated At</span>
                <div className="flex items-center gap-1.5 text-sm text-slate-700">
                   <Calendar className="h-3.5 w-3.5 text-slate-400" />
              ...
              <div>
                ...
                <div className="flex items-center gap-1.5 text-sm text-slate-700">
                   ...
                   {user.lastLoginAt ? formatDateTimeVN(user.lastLoginAt) : "Never"}
                </div>
              </div>
            </div>
          </div>
